...
export async function onRequest(context) {
  ...
  for (const item of files) {
    ...
    // Keep keys single-segment for Pages Functions params compatibility.
    const objectKey = `social-${crypto.randomUUID()}.${String(ext || "bin").toLowerCase()}`;
    ...
    uploaded.push({
      ...
      contentType: item.type || "application/octet-stream",
      size: item.size,
      viewUrl: `/api/uploads/social/${encodeURIComponent(objectKey)}`,
    });
  }
